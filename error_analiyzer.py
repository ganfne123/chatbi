...
该模块用于系统性地收集、分类和追踪 Text2SQL 系统的错误模式
"""
...
class ErrorAnalyzer:
    ...
    def categorize_error(
        ...
        if detected_types:
            ...
            result["suggestion"] = (
                "将该用例加入 Few-shot 示例或增强 Schema 描述中的相关字段说明"
            )
    ...
    def _get_suggestion(self, error_type: str) -> str:
        ...
        suggestions = {
            self.FIELD_ERROR: (
                "在 RULES 中强化字段语义定义，增加字段选择错误的 Few-shot 负例"
            ),
            self.JOIN_ERROR: (
                "在 ERROR_GUARDS 中补充 Join 路径检查清单，增加多表查询 Few-shot 示例"
            ),
            self.TIME_ERROR: (
                "增加时间计算相关的 Few-shot 示例，明确动态时间边界的 SQL 写法"
            ),
            self.FILTER_ERROR: (
                "在 RULES 中明确各类查询的强制过滤条件，增加过滤遗漏的负例"
            ),
            self.AGGREGATION_ERROR: (
                "增加 GROUP BY 正确用法的示例，强调 SELECT 非聚合字段必须出现在 GROUP BY 中"
            ),
            self.SYNTAX_ERROR: (
                "检查 Prompt 中是否有格式混乱导致模型输出不完整 SQL 的情况"
            ),
        }
